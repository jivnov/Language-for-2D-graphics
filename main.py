...
class MyTwoDimParserListener(TwoDimParserListener):
    ...
    def enterDrawClause(self, ctx: TwoDimParser.DrawClauseContext):
        ...
        self.res.canvas.save()
        print(
            f"Entered draw clause! Drawing shape {ctx.IDENTIFIER()}")  # Here identifier is a single value as
        # drawClause can have 0 or 1 IDENTIFIERs passed to it (check the TwoDimParser.g4 rule)
