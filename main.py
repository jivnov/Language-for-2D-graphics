...
class MyTwoDimParserListener(TwoDimParserListener):
    ...
    def enterShapeSpec(self, ctx: TwoDimParser.ShapeSpecContext):
        print(f"Planning/declaring shapes {ctx.typeName().}")  # Here IDENTIFIER is a list, because you can pass multiple IDENTIFIERs in a single shapeSpec call
    ...
    def enterViewportClause(self, ctx: TwoDimParser.ViewportClauseContext):
        ...
        res = drawing.Drawing2d(int(ctx.DECIMAL_LIT(0).getText()),int(ctx.DECIMAL_LIT(1).getText()))
        res.canvas.saveSvg('example.svg')
